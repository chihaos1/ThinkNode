...
export default function Node({ id, position, label, isSelected = false}: NodeProps) {
    ...
    const handleClick = () => {
        window.dispatchEvent(new CustomEvent("node-clicked", {detail: { id, label, position }}))
    }
    ...
    return (
        <group ref={nodeRef} position={position}>
            ...
            <lineSegments>
                ...
                <lineBasicMaterial color="#705d42" transparent opacity={0.9} />
            </lineSegments>
            <Html 
                center
                distanceFactor={12}>
